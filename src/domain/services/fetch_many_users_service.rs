...
impl<UserRepository: UserRepositoryTrait> FetchManyUsersService<UserRepository> {
    ...
    pub async fn exec(&self, params: FetchManyUsersParams) -> Result<FetchManyUsersResponse, Box<dyn DomainErrorTrait>> {
        ...
        let response = self.user_repository.find_many(PaginationParameters {
            items_per_page,
            page,
            query: params.query
        }).await;
...
mod test {
    ...
    async fn test() {
        ...
        .returning(move |params| {
            ...
            Ok(FindManyUsersResponse (res_users, total_before_paginating as u64))
        });

        // TESTING

        let sut = FetchManyUsersService::new(
            Box::new(mocked_user_repository)
        );

        let result_1 = sut.exec(FetchManyUsersParams {
            page: None,
            per_page: None,
        ...
        let result_2 = sut.exec(FetchManyUsersParams {
            page: None,
            per_page: None,
            query: None,
        }).await;
