...
mod test {
    ...
    async fn test() {
        ...
        let team_role_db: Arc<Mutex<Vec<TeamRole>>> = Arc::new(Mutex::new(vec![
            team_role_1.clone(),
            team_role_2.clone()
        ]));
        ...
        .returning(move |params| {
            ...
            Ok(FindManyTeamRolesResponse (res_team_roles, total_before_paginating as u64))
        });

        // TESTING

        let sut = FetchManyTeamRolesService::new(
            Box::new(mocked_team_role_repository)
        );
        ...
        let result_2 = sut.exec(FetchManyTeamRolesParams {
            page: None,
            per_page: None,
            query: None,
        }).await;
