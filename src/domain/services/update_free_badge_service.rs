...
impl<FreeBadgeRepository: FreeBadgeRepositoryTrait> UpdateFreeBadgeService<FreeBadgeRepository> {
    ...
    pub async fn exec(&self, params: UpdateFreeBadgeParams) -> Result<FreeBadge, Box<dyn DomainErrorTrait>> {
        let user_can_edit_free_badge = verify_role_has_permission(&params.user_role, RolePermissions::UpdateFreeBadge);

        if !user_can_edit_free_badge {
            return Err(Box::new(UnauthorizedError::new()));
        }

        let result = self.free_badge_repository.find_by_id(params.free_badge_id).await;
        ...
        let free_badge = result.unwrap();

        if free_badge.is_none() {
            return Err(Box::new(BadRequestError::new()));
        }

        let mut free_badge = free_badge.unwrap();
        ...
        let free_badge =  self.free_badge_repository.save(free_badge).await;

        if free_badge.is_err() {
            return Err(generate_service_internal_error(
                "Error occurred in Update Free Badge Service on updating the free badge in the database".into(),
...
mod test {
    ...
    async fn test_if_authorized_user_can_edit_free_badge() {
        ...
        let free_badge = FreeBadge::new("BR001".into(), "i.imgur.com".into(), "www.cosmic.com/news/a".into(), false, None);
        badges_db.lock().unwrap().push(free_badge.clone());

        let badge_deadline = TimeHelper::now() + chrono::Days::new(5);

        let result = sut.exec(UpdateFreeBadgeParams {
            ...
            available_until: Some(Some(badge_deadline.clone())),
        }).await;
        ...
        assert_eq!(&"habbo.com/imager/badges/BR001".to_string(), badges_db.lock().unwrap()[0].image());
    }
