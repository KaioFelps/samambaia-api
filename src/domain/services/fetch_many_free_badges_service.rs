...
impl<FreeBadgeRepository: FreeBadgeRepositoryTrait> FetchManyFreeBadgesService<FreeBadgeRepository> {
    ...
    pub async fn exec(&self, params: FetchManyFreeBadgesParams) -> Result<FetchManyFreeBadgesResponse, Box<dyn DomainErrorTrait>> {
        let page = params.page.unwrap_or(1);
        let items_per_page = params.per_page.unwrap_or(DEFAULT_PER_PAGE as u32);

        let result = self.free_badge_repository.find_many(PaginationParameters {
            page,
            items_per_page,
            query: None,
        }).await;
        ...
        if result.is_err() {
            return Err(generate_service_internal_error(
                "Error occurred in Fetch Many Free Badges service, on fetching many free badges from database".into(),
                &result.unwrap_err()
            ));
        }
...
mod test {
    ...
    async fn test_if_can_get_paginated_free_badges_list() {
        ...
        *badges_db.lock().unwrap() = vec![
            FreeBadge::new("PT001".to_string(), "i.imgur.com".to_string(), "www.cosmic.com/news/1".to_string(), false, None),
            FreeBadge::new("PT002".to_string(), "i.imgur.com".to_string(), "www.cosmic.com/news/2".to_string(), false, Some(TimeHelper::now() + chrono::Days::new(2))),
            FreeBadge::new("PT003".to_string(), "i.imgur.com".to_string(), "www.cosmichotel.com/".to_string(), true, Some(TimeHelper::now() + chrono::Days::new(4))),
            FreeBadge::new("PT004".to_string(), "i.imgur.com".to_string(), "www.cosmic.com/news/10".to_string(), false, Some(TimeHelper::now() + chrono::Days::new(3))),
        ];

        let sut = super::FetchManyFreeBadgesService::new(badges_repository);

        let result = sut.exec(FetchManyFreeBadgesParams {
            per_page: Some(3),
            page: None,
        }).await;

        assert!(result.is_ok(), "Expected service to have been successfully performed.");

        let result = result.unwrap();

        assert_eq!(result.pagination.total_items, 4, "Expected it to be 4 items on the mocked database.");
        assert_eq!(result.pagination.total_pages, 2, "Expected response to have two pages.");
        assert_eq!(result.pagination.current_page, 1, "Expected current page to be 1 by default.");
