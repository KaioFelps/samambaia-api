...
pub async fn exec() -> Either<DeleteArticleTagService<SeaArticleTagRepository>, HttpResponse> {
    ...
    let delete_article_tag_service = DeleteArticleTagService::new(
        article_tag_repository,
    );

    Left(delete_article_tag_service)
}