...
pub struct __SeaUserRepository<'a> {
    pub sea_service: &'a SeaService,
}

impl __SeaUserRepository<'_> {
    pub async fn new(sea_service: &SeaService) -> __SeaUserRepository<'_> {
        __SeaUserRepository { sea_service }
    }
}

#[async_trait]
impl UserRepositoryTrait for __SeaUserRepository<'_> {
    async fn create(&self, user: User) -> Result<User, Box<dyn Error>> {
        let new_user = SeaUserMapper::user_to_sea_active_model(user);
