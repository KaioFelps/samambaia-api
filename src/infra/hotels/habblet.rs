...
pub struct HabbletUtilitiesExternalServices<'this> {
    api_url: &'this str,
}

impl<'this> HabbletUtilitiesExternalServices<'this> {
...
#[async_trait]
impl<'this> HotelUtilitiesExternalService for HabbletUtilitiesExternalServices<'this> {
    async fn get_user_motto(&self, nickname: &str) -> Result<Option<String>, SamambaiaError> {
        ...
        let user_info = response.json::<UserMinimalData>().await.map_err(|err| {
            generate_service_internal_error(
                "Habblet API has not returned a valid JSON on user data fetch",
                Box::new(err),
            )
        })?;
