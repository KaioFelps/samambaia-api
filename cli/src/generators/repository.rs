...
use crate::{helpers::{extract_dir_flag, get_capitalized_name}, templates::get_repository_flat_template};

pub fn generate_repository(args: &Vec<String>, current_dir: &PathBuf) {
    ...
    match custom_final_path {
        None => repositories_dir = current_dir.join("src/domain/repositories/"),
        Some(dir) => repositories_dir = current_dir.join(Path::new(&dir))
    }
    ...
    match repository_file {
        ...
        Ok(mut file) => {
            if let Err(err) = file.write(get_repository_flat_template(&repository_capitalized_name).as_bytes()) {
                eprintln!("Error on creating {}.rs: {}",repository_file_name, err);
                return;
            }

            println!("Created repository {}.rs on {}.", repository_file_name, repositories_dir.display().to_string());
        }
    };

    let repositories_mod_file = OpenOptions::new().create(true).append(true).open(repositories_dir.join("mod.rs"));

    match repositories_mod_file {
        Err(err) => {
            eprintln!("Error on adding repository {}.rs to mod.rs: {}", repository_file_name, err)
        },
