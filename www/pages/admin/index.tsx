...
import { SummaryCard, type SummaryCardProps } from "@/ui/admin/summary-card";
...
export default function AdminHome({ summary: _summary, councilAlerts }: AdminHomeProps) {
  ...
  return (
    <main className="admin-main-container">
      <section id="summary" className="flex gap-3 p-6 bg-white/40 rounded-lg -mx-3 mb-12">
        ...
        <div className="flex flex-col gap-6">
          {councilAlerts.length ? (
            councilAlerts.map((alert) => (
              <CouncilAlertCard key={`council-alert-card-${alert.id}`} {...alert} />
            ))
          ) : (
            <Alert admin type="info" message="Não há nenhum aviso do grêmio." />
          )}
        </div>
      </section>
    </main>
