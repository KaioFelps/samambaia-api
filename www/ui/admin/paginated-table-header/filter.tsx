...
export const PaginatedTableHeaderFilter = memo(({
  ...
  handleFilter,
  loading,
}: PaginatedTableHeaderFilterProps) => {
  const [filter, setFilter] = useState<string | undefined>();
  const [query, setQuery] = useState<string | undefined>();

  const pageUrl = usePage().url;

  const handleSubmitFilter = useCallback((event: FormEvent) => {
  ...
  useEffect(() => {
    const search = Object
      .entries(PaginationPolitics.getQueryObjectFromUrl(pageUrl))
      .find(([key]) => filters
        .some(filter => filter.value === key));
